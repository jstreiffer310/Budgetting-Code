...
function updateInvestmentValues() {
  ...
  // Assume columns: Account, Ticker, Shares, Price, Value
  const rows = holdingsSheet.getRange(2, 1, holdingsSheet.getLastRow() - 1, 5).getValues();
  let updatedRows = [];
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const shares = parseFloat(rows[i][2]);
    let price = 0;
    let value = 0;

    if (!acct || !ticker || isNaN(shares)) {
      updatedRows.push([acct, ticker, shares, rows[i][3], rows[i][4]]);
      continue;
    }
    ...
    try {
      ...
      if (ticker.endsWith("-CAD")) {
        // Crypto: Use Yahoo Finance or fallback to CoinGecko API
        url = `https://finance.yahoo.com/quote/${ticker}`;
        const res = UrlFetchApp.fetch(url, { muteHttpExceptions: true });
        const content = res.getContentText();
        priceMatch = content.match(/"regularMarketPrice":\{"raw":([\d\.]+),/);
        if (priceMatch && priceMatch[1]) {
          price = parseFloat(priceMatch[1]);
        } else {
          // Try CoinGecko API for crypto
          let cgId = ticker.replace("-CAD", "").toLowerCase();
          let cgUrl = `https://api.coingecko.com/api/v3/simple/price?ids=${cgId}&vs_currencies=cad`;
          const cgRes = UrlFetchApp.fetch(cgUrl, { muteHttpExceptions: true });
          const cgJson = JSON.parse(cgRes.getContentText());
          if (cgJson[cgId] && cgJson[cgId].cad) {
            price = parseFloat(cgJson[cgId].cad);
          }
        }
      } else {
      ...
      Logger.log(`Error fetching price for ${ticker}: ${err.message}`);
    }
    ...
    updatedRows.push([acct, ticker, shares, price, value]);
  }

  // Write back updated prices and values
  if (anyUpdated) {
    holdingsSheet.getRange(2, 1, updatedRows.length, 5).setValues(updatedRows);
    SpreadsheetApp.getUi().alert("Holdings prices and values updated.");
  } else {
    SpreadsheetApp.getUi().alert("No prices found for holdings. Check tickers and internet connection.");
  }
}
...
function suggestCategoriesFromTransactions() {
  ...
  // Get merchant names and Interac sender/recipient names for suggestions
  ...
  txData.forEach(row => {
    // row: [Date, Amount, From, To, Bank, Notes, ID, Category, Type]
    const bank = (row[4] || "").toLowerCase();
    const to = (row[3] || "").toLowerCase();
    const notes = (row[5] || "").toLowerCase();

    // Purchases: use merchant name
    if (bank.includes("purchase") || bank.includes("credit card")) {
      let merchant = to.replace(/[\*#\d-]/g, ' ').trim();
      if
